    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if(batLevel<=20){
            if (Constants.isNightModeActive(context)){
                ...
                Toast.makeText(context, "Dark Mode Enabled. Battery Level is Low", Toast.LENGTH_LONG);
            }
        }
        else if (batLevel>20 && isInAutoDarkMode){
            AppCompatDelegate.setDefaultNightMode(AppCompatDelegate.MODE_NIGHT_NO);
            isInAutoDarkMode = false;
            Toast.makeText(context, "Dark Mode automatically Disabled", Toast.LENGTH_LONG);
        }
    }
}
